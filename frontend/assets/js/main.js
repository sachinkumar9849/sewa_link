...
(function () {
    ...
    const countDown = new Date(birthday).getTime(),
        ...
        //   document.getElementById("days").innerText = Math.floor(distance / (day)),
            document.getElementById("hours_a").innerText = Math.floor((distance % (day)) / (hour)),
            document.getElementById("minutes_a").innerText = Math.floor((distance % (hour)) / (minute)),
            document.getElementById("seconds_a").innerText = Math.floor((distance % (minute)) / second);
...
(function($) {
	...
	var imgCheckboxClass = function(element, options, id) {
		...
		if (options.fixedImageSize)
		{
			...
			$.extend(true, $finalStyles,{ "span.imgCheckbox .img_select": {
